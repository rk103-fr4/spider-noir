...
def collect_inputs() -> dict:
    ...
    # El rate limit protege al target y evita bloqueos de IP.
    # ══════════════════════════════════════════════════════════════════════════
    print("\n\033[38;5;220m  ╔═══════════════════════════════════════════════════════════════════╗\033[0m")
    print("\033[38;5;220m  ║  BLOQUE 0a — PERFIL DEL TESTER                                   ║\033[0m")
    print("\033[38;5;220m  ║  Headers de identificación + rate limit para Bug Bounty.         ║\033[0m")
    print("\033[38;5;220m  ║  Se aplica a Katana, ffuf y subfinder automáticamente.           ║\033[0m")
    print("\033[38;5;220m  ╚═══════════════════════════════════════════════════════════════════╝\033[0m\n")

    config = {}

    # ── Header de identificación (ej: X-HackerOne-Research: tu-usuario) ──────
    ...
    headers_input = input(
        ...
        "      ej: X-HackerOne-Research: tu-usuario\n"
        "      ej: X-HackerOne-Research: tu-usuario, X-Custom: valor > "
    ).strip()
    ...
    #   Sin límite (lab / local)      : 0 (usa defaults de cada herramienta)
    print()
    print("  \033[38;5;240m  El rate limit protege al target y evita bloqueos de IP.\033[0m")
    ...
    if config["rate_limit"] > 0:
        ...
        log("INFO", "Sin rate limit: Katana -c 20 | ffuf -t 50 (defaults)")
    ...
    # ══════════════════════════════════════════════════════════════════════════
    print("\n\033[93m━━━━━━━━━━━━━━━━━━━━━━━━━  TARGET GLOBAL  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m")
    print("  URL o IP que actúa como punto de entrada para todo el pipeline.\n")

    while True:
        target = input("  \033[96m[?]\033[0m URL o IP Objetivo (ej: http://10.10.11.20 o http://target.htb): ").strip()
        if not target:
            print("      \033[91m[!] El objetivo no puede estar vacío.\033[0m")
            continue
    ...
    port_input = input("  \033[96m[?]\033[0m Puerto [Enter = 80 para http / 443 para https]: ").strip()
    if port_input.isdigit():
        ...
        log("INFO", f"Puerto inferido del esquema: {config['port']}")

    # ══════════════════════════════════════════════════════════════════════════
    # BLOQUE 0c — SCOPE DESDE ARCHIVO (HackerOne CSV o Burp JSON)
    ...
    print("\033[38;5;39m  ║  BLOQUE 0c — SCOPE DESDE ARCHIVO                                 ║\033[0m")
    print("\033[38;5;39m  ║  Cargá el CSV de HackerOne o el JSON de Burp Suite.              ║\033[0m")
    print("\033[38;5;39m  ║  Se extraen wildcards y URLs in-scope automáticamente.           ║\033[0m")
    print("\033[38;5;39m  ╚═══════════════════════════════════════════════════════════════════╝\033[0m\n")
    ...
    scope_file_input = input(
        "  \033[96m[?]\033[0m Ruta al archivo de scope (Enter para omitir)\n"
    ...
    if scope_file_input:
        ...
        if not scope_path.exists():
            log("WARN", f"Archivo no encontrado: {scope_path}")
        else:
            # Detectar formato por extensión
            ext = scope_path.suffix.lower()
            if ext == ".csv":
                log("INFO", f"Parseando CSV de HackerOne: {scope_path}")
                parsed = parse_hackerone_csv(str(scope_path))
                fmt    = "HackerOne CSV"
            elif ext in (".json", ".burp"):
                log("INFO", f"Parseando scope de Burp Suite: {scope_path}")
                parsed = parse_burp_scope(str(scope_path))
                fmt    = "Burp Suite JSON"
            else:
                # Intentar como CSV por defecto
                log("INFO", f"Extensión desconocida, intentando como CSV: {scope_path}")
                parsed = parse_hackerone_csv(str(scope_path))
            ...
            log("OK", f"Scope cargado desde {fmt}:")
            ...
            log("INFO", f"  URLs exactas in-scope: {len(parsed['exact_urls'])}")
    ...
    print("\033[38;5;208m  ║  BLOQUE 0b — SUBFINDER  (Enumeración de subdominios)             ║\033[0m")
    print("\033[38;5;208m  ║  Descubre admin., api., dev., staging. antes de crawlear.        ║\033[0m")
    print("\033[38;5;208m  ║  Muy recomendado para Bug Bounty. Opcional para HTB.             ║\033[0m")
    print("\033[38;5;208m  ╚═══════════════════════════════════════════════════════════════════╝\033[0m\n")
    ...
    if use_subfinder in ("y", "yes", "s", "si", "sí") and not subfinder_available:
        ...
        try:
            ...
            log("WARN", "Valor inválido, usando límite por defecto: 20")
        log("OK", f"Límite de subdominios: {config['subfinder_limit'] or 'Sin límite'}")

    else:
        log("INFO", "subfinder omitido. Continuando sin enumeración de subdominios.")
        config["subfinder_keywords"] = []
        config["subfinder_limit"]    = 20
    ...
    print("\033[38;5;51m  ║  BLOQUE 1 — KATANA  (Crawler)                                    ║\033[0m")
    print("\033[38;5;51m  ║  Katana sigue links del HTML/JS automáticamente. No usa wordlist. ║\033[0m")
    print("\033[38;5;51m  ╚═══════════════════════════════════════════════════════════════════╝\033[0m\n")

    # Mostrar los VHOSTs ya cargados por subfinder (si corrió)
    subfinder_vhosts = config.get("subfinder_results", [])
    if subfinder_vhosts:
        print(f"  \033[92m[+]\033[0m Subfinder ya agregó {len(subfinder_vhosts)} subdominio(s) al crawling:")
    ...
    print()
    timeout_in = input(
        "  \033[96m[?]\033[0m Timeout global de Katana por scope en segundos [300]\n"
        "      Targets con WAF agresivo: 60-120s  |  Targets permisivos: 300s+\n"
        "      Si no llegan URLs en 45s → el scope se abandona automáticamente > "
    ).strip()
    try:
    ...
    print("\033[38;5;214m  ║  BLOQUE 2 — FFUF  (Fuzzer de directorios y archivos)             ║\033[0m")
    print("\033[38;5;214m  ║  Pasada 1: wordlist de directorios  →  /FUZZ                     ║\033[0m")
    ...
    print("  \033[38;5;214m── Pasada 1: Directorios (/FUZZ) ───────────────────────────────────\033[0m\n")
    while True:
        wordlist = input(
            "  \033[96m[?]\033[0m Wordlist de DIRECTORIOS (obligatoria)\n"
            "      Palabras pensadas como rutas: admin, backup, api, config\n"
            "      [Enter = buscar wordlist por defecto en el sistema]: "
        ).strip()
        if not wordlist:
            defaults = [
                "/usr/share/wordlists/dirb/common.txt",
            ...
            for d in defaults:
                if Path(d).exists():
                    wordlist = d
                    log("INFO", f"Wordlist de directorios por defecto: {wordlist}")
                    break
            else:
                print("      \033[91m[!] No se encontró wordlist por defecto. Especificá una ruta.\033[0m")
                continue
        if not Path(wordlist).is_file():
            print(f"      \033[91m[!] Archivo no encontrado: {wordlist}\033[0m")
            continue
    ...
    print()
    ac_input = input(
        "  \033[96m[?]\033[0m ¿Activar auto-calibración de ffuf? (-ac) [S/n]\n"
        "      S → filtra falsos positivos automáticamente (recomendado para HTB)\n"
    ...
    ext_input = input(
        ...
        "      Archivos sensibles     : .bak,.sql,.log,.zip,.tar.gz\n"
        "      > "
    ).strip()
    ...
    if ext_input:
        ...
        log("OK", f"Extensiones configuradas: {exts}")
    ...
    print("\033[38;5;99m  ║  BLOQUE 3 — SCOPE  (aplica a Katana)                             ║\033[0m")
    ...
    if file_wildcards or file_exact:
        print(f"  \033[92m[+]\033[0m Scope pre-cargado desde archivo:")
        if file_wildcards:
    ...
    log("OK", f"Dominios exactos totales: {len(scope_domains)} "
    ...
    print()
    wc_in = input(
        "  \033[96m[?]\033[0m [B] Wildcards adicionales (Enter = usar los del archivo)\n"
    ...
    if config["scope_wildcards"]:
        log("OK", f"Wildcards totales: {len(config['scope_wildcards'])}")
    ...
    print()
    rx_in = input(
        "  \033[96m[?]\033[0m [C] Regex avanzada de scope (opcional)\n"
        "      Se evalúa contra la URL COMPLETA (dominio + path).\n"
        "      Útil cuando necesitás filtrar por path, no solo por dominio.\n"
        "      Si A y B ya cubren tu scope, dejá esto vacío.\n"
        "\n"
        "      Ejemplos de cuándo usarla:\n"
        "        Solo endpoints de API  →  .*/api/.*\n"
        "        Cualquier .htb         →  .*\\.htb.*\n"
        "        Solo HTTPS             →  ^https://.*\n"
        "        Un path específico     →  .*/v2/users.*\n"
        "\n"
    ...
    # Los nodos de ffuf nunca se colapsan (son hallazgos críticos individuales).
    ...
    try:
        ...
        log("WARN", "Valor inválido, usando profundidad por defecto: 2")
    log("OK", f"Profundidad del grafo: {config['graph_depth']} niveles")
    ...
    try:
        ...
        log("WARN", "Valor inválido, usando límite por defecto: 300")
    lim_label = str(config["graph_node_limit"]) if config["graph_node_limit"] else "Sin límite"
    log("OK", f"Límite de nodos del grafo: {lim_label}")
    ...
    _hdrs = " | ".join(config.get("custom_headers", [])) or "Ninguno"
    _rl   = f"{config.get('rate_limit')} req/s" if config.get('rate_limit') else "Sin límite"
    _nf   = "SÍ ⚠" if config.get('no_forms') else "No"
    print(f"  \033[38;5;220m[PERFIL ]\033[0m  Headers: \033[96m{_hdrs}\033[0m")
    ...
    confirm = input("  \033[96m[?]\033[0m ¿Confirmar y lanzar el pipeline? (S/n): ").strip().lower()
    if confirm in ("n", "no"):
        log("WARN", "Pipeline cancelado por el usuario.")
        sys.exit(0)
...
def run_katana(target_url: str, vhost: "str | Ninguno", output_dir: Path, config: dict) -> list:
    ...
    if config.get("custom_headers"):
        log("INFO", f"Headers de identificación añadidos a Katana: {config['custom_headers']}")
    ...
    # ── Modo fallback sin Rich: subprocess.run clásico ───────────────────────
        ...
        try:
            ...
            log("ERROR", "'katana' no encontrado en PATH.")
            return []
    ...
    log("OK", f"Katana → {len(discovered)} URLs únicas in-scope en {elapsed:.1f}s para '{label}'{scope_info}")
    return discovered
...
def run_ffuf(target_url: str, vhost: "str | Ninguno", wordlist: str,
    ...
    for header in config.get("custom_headers", []):
        cmd.extend(["-H", header])
    if config.get("custom_headers"):
        log("INFO", f"Headers de identificación añadidos a ffuf: {config['custom_headers']}")
    if vhost:
        cmd.extend(["-H", f"Host: {vhost}"])
    ...
    log("INFO", f"Wordlist: {total_words:,} palabras a probar")
    ...
    # ── Modo fallback sin Rich ───────────────────────────────────────────────
        ...
        try:
            ...
            log("ERROR", "'ffuf' no encontrado en PATH.")
            return []
...
def run_ffuf_all_passes(target_url: str, vhost: "str | Ninguno",
    ...
    Returns:
        Lista consolidada de todos los hallazgos de todas las pasadas.
    """
...
def build_graph(config: dict, katana_results: dict, ffuf_results: dict) -> Network:
    ...
    """
    log("PHASE", "Fase 3: Construyendo el Grafo Jerárquico (PyVis)...")
    ...
    <div style='background:#1a1a2e;color:#FFD700;padding:12px;border-radius:8px;
        ...
        <b style='font-size:14px;'>🎯 OBJETIVO RAÍZ</b>
    ...
    for subdomain in config.get("subfinder_results", []):
        ...
        if f"vhost:{subdomain}" not in added_nodes:
            ...
            <div style='background:#1a1a2e;color:#8E44AD;padding:12px;border-radius:8px;
                ...
                <b>🔍 SUBDOMINIO (subfinder)</b>
                <hr style='border-color:#8E44AD;margin:6px 0;'>
                <b>Subdominio:</b> {subdomain}<br>
                <b>Origen:</b> Enumeración pasiva<br>
                <i style='color:#888;font-size:10px;'>Descubierto sin contactar el target</i>
            </div>
            """
            add_node_safe(
    ...
    # ─────────────────────────────────────────────────────────────────────────
    log("INFO", "Construyendo árbol jerárquico de paths de Katana...")
    ...
    for scope_label, urls in katana_results.items():
        ...
        # Aplicar cuota del scope
        if node_limit and len(scope_nodes) > quota:
            log("INFO", f"  {scope_label}: {len(scope_nodes)} nodos → {quota} (cuota del scope)")
            scope_nodes = scope_nodes[:quota]
        else:
            log("INFO", f"  {scope_label}: {len(scope_nodes)} nodos (dentro de la cuota)")

        katana_nodes_by_scope[scope_label] = {pk: node for pk, node in scope_nodes}
        total_katana_inserted += len(scope_nodes)

    log("INFO", f"Nodos Katana a insertar: {total_katana_inserted} "
    ...
    for scope_label, urls in katana_results.items():
        ...
        for path_key in sorted_paths:
            ...
            <div style='background:#0d1117;color:#58A6FF;padding:12px;border-radius:8px;
                ...
                <b>URLs agrupadas:</b> <span style='color:#FFD700'>{count}</span><br>
                <b>Hijos:</b> {len(children)}<br>
    ...
    log("INFO", f"Árbol Katana: {katana_node_count} nodos (profundidad {max_depth})")
    ...
    # ─────────────────────────────────────────────────────────────────────────
    log("INFO", "Añadiendo hallazgos de ffuf...")
    ...
    for scope_label, paths in ffuf_results.items():
        ...
        for entry in paths:
            ...
            <div style='background:#0d1117;color:#FF8C00;padding:12px;border-radius:8px;
                ...
                <hr style='border-color:{border_color};margin:6px 0;'>
                <i style='color:#888;font-size:10px;'>💡 Investigar manualmente</i>
            </div>
            """
            ...
            # para que los hallazgos de ffuf aparezcan en la rama correcta
    ...
    log("OK", f"Grafo construido: \033[96m{total_nodes}\033[0m nodos, \033[96m{total_edges}\033[0m edges")
...
def export_graph(net, config: dict) -> Path:
    ...
    log("PHASE", f"Exportando grafo SVG a: {output_file}")
...
<div id="main">
  <div id="canvas-wrap">
    ...
    <div id="legend">
      <h4>📊 LEYENDA</h4>
      <div class="li"><span style="color:#FFD700;font-size:15px">★</span> Raíz</div>
      ...
      <div style="margin-top:6px;border-top:1px solid #222;padding-top:5px;
                  color:#333;font-size:9px;">Color = tecnología detectada</div>
    </div>
  </div>
  <div id="panel">
    <div id="ph">⚡ PANEL DE ACCIONES</div>
    <div id="ni">
      <div class="ni-empty">← Hacé click en un nodo<br>para ver sus detalles<br>y acciones disponibles</div>
    </div>
    <div id="al"></div>
  </div>
</div>
...
<div id="modal"><div id="mbox">
  <h3>⚡ Comando copiado al portapapeles</h3>
  <div id="mcmd"></div>
  <div id="mhint">Pegá en tu terminal con Ctrl+Shift+V</div>
...
function showPanel(nodeId) {{
    ...
    ni.innerHTML =
        ...
        (hasForms ? '<div style="margin:3px 0;color:#E91E63">📝 Formularios detectados</div>' : '') +
        '<div style="margin-top:6px"><span style="color:#333;font-size:9px">' + (n.group||'') + '</span></div>';

    al.innerHTML = '';
    al.appendChild(mkGrp('RECONOCIMIENTO'));
    al.appendChild(mkBtn('🌐', 'Abrir en browser',   A.browser(url), 'info'));
    ...
    if (g==='seed'||g==='vhost'||g==='subfinder') {{
        ...
        al.appendChild(mkGrp('VULNERABILIDADES'));
...
renderAll();
    ...
    log("OK", f"Grafo exportado: \033[96m{output_file.resolve()}\033[0m")
    log("INFO", f"Abrí el grafo con: xdg-open {output_file.resolve()}")
    return output_file
    ...
    if RICH_AVAILABLE:
        with Progress(
            ...
            try:
                ...
                log("INFO",  "Instalación: go install github.com/projectdiscovery/subfinder/v2/cmd/subfinder@latest")
                return []
    else:
        try:
            result = subprocess.run(
...
def run_pipeline(config: dict):
    ...
    print("\n\033[93m━━━━━━━━━━━━━━━━━━━━━━  VERIFICACIÓN DE DEPENDENCIAS  ━━━━━━━━━━━━━━━━━━\033[0m")
    ...
    if config.get("use_subfinder") and subfinder_ok:
        print("\n\033[38;5;208m━━━━━━━━━━━━━━━━━━━━━━  FASE 0: SUBFINDER  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━\033[0m")
    ...
    # ─────────────────────────────────────────────────────────────────────────
    print("\n\033[95m━━━━━━━━━━━━━━━━━━━━━━  FASE 1: KATANA CRAWLING  ━━━━━━━━━━━━━━━━━━━━━━\033[0m")
    ...
    if not katana_ok:
        ...
        for target_url, vhost in katana_scopes:
            ...
            log("PHASE", f"Crawleando scope: {scope_label}")
            urls = run_katana(target_url, vhost, output_dir, config)
            katana_results[scope_label] = urls
            log("INFO", f"  → {len(urls)} URLs añadidas para '{scope_label}'")

    total_katana = sum(len(v) for v in katana_results.values())
    log("OK", f"Fase 1 completa. Total URLs de Katana: \033[96m{total_katana}\033[0m")

    # ─────────────────────────────────────────────────────────────────────────
    # FASE 2: ffuf Fuzzing — con estimación previa y grafo parcial en Ctrl+C
    # ─────────────────────────────────────────────────────────────────────────
    print("\n\033[95m━━━━━━━━━━━━━━━━━━━━━━  FASE 2: FFUF FUZZING  ━━━━━━━━━━━━━━━━━━━━━━━━\033[0m")
    ...
    if not ffuf_ok:
        ...
        print(f"  \033[93m│\033[0m  ETA estimado  : \033[96m{eta_str}\033[0m")
        ...
        if eta_seconds > 1800:
            ...
            log("INFO", "  También podés cancelar con Ctrl+C — el grafo se generará igual.")
            print()
        ...
        if confirm_ffuf in ("n", "no"):
            ...
            try:
                ...
                ffuf_cancelled = True
                print()
                log("WARN", "ffuf interrumpido por el usuario (Ctrl+C).")
                log("INFO", f"  Scopes completados: {len(ffuf_results)}/{len(ffuf_scopes)}")
                log("INFO", "  Generando grafo parcial con los datos disponibles...")

    total_ffuf = sum(len(v) for v in ffuf_results.values())
    status_ffuf = "PARCIAL (cancelado)" if ffuf_cancelled else "completo"
    ...
    # ─────────────────────────────────────────────────────────────────────────
    print("\n\033[95m━━━━━━━━━━━━━━━━━━━━━  FASE 3: CONSOLIDACIÓN DEL GRAFO  ━━━━━━━━━━━━━━\033[0m")
    if ffuf_cancelled:
        log("INFO", "Grafo parcial: incluye Katana completo + ffuf hasta donde llegó.")
    ...
    # RESUMEN FINAL
    # ─────────────────────────────────────────────────────────────────────────
    parcial_label = " \033[93m(GRAFO PARCIAL — ffuf cancelado)\033[0m" if ffuf_cancelled else ""
    print(f"\n\033[92m━━━━━━━━━━━━━━━━━━━━━━━━━━  PIPELINE COMPLETO{parcial_label}  ━━━━━━━━━━━━━━━━━━━\033[0m")
    log("OK", f"Directorio de sesión : \033[96m{output_dir.resolve()}\033[0m")
    log("OK", f"Grafo HTML generado  : \033[96m{output_html.resolve()}\033[0m")
    log("OK", f"Nodos totales        : \033[96m{len(net.nodes)}\033[0m")
    log("OK", f"Edges totales        : \033[96m{len(net.edges)}\033[0m")
    log("OK", f"URLs de Katana       : \033[96m{total_katana}\033[0m")
    log("OK", f"Rutas de ffuf        : \033[96m{total_ffuf}\033[0m")
